...
import { CreditCard, Search, RefreshCw, Users, Clock, CheckCircle, XCircle, Coins, DollarSign, Receipt } from 'lucide-react';
...
export function AdminPaymentManager() {
  ...
  const loadData = async () => {
    setLoading(true);
    await Promise.all([loadPayments(), loadSubscriptions(), loadBankTransfers()]);
    setLoading(false);
  };
  ...
  const loadPayments = async () => {
    const { data, error } = await supabase
      ...
      .select('id, user_id, amount, status, subscription_type, toss_order_id, payment_key, payment_method, created_at')
      .order('created_at', { ascending: false });

    if (error) { console.error('Payment load error:', error); return; }
  ...
  const loadSubscriptions = async () => {
    ...
    if (error) { console.error('Subscription load error:', error); return; }
  ...
  const loadBankTransfers = async () => {
    ...
    if (error) { console.error('Bank transfer load error:', error); return; }
  ...
  const filteredPayments = payments.filter(p => {
    const matchesSearch = !searchTerm ||
      ...
      p.toss_order_id?.includes(searchTerm);
    const matchesFilter = paymentFilter === 'all' ||
      (paymentFilter === 'completed' && p.status === 'completed') ||
  ...
  const getPaymentTypeName = (type: string) => {
    switch (type) {
      ...
      case 'single_test': return '단건검사';
      case 'single_report': return '단건리포트';
      case 'single': return '단건상담';
      case 'pass': return '이용권';
      case 'cash': case 'token': return '캐시충전';
      default: return type;
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed': return <Badge className="bg-green-100 text-green-700 border-green-200"><CheckCircle className="w-3 h-3 mr-1" />완료</Badge>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        ...
        <Card className="border-gray-100">
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold">{completedPayments.length}건</p>
          </CardContent>
        </Card>
        <Card className="border-gray-100">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-500">결제 대기</span>
            </div>
            <p className="text-xl font-bold">{pendingPayments.length}건</p>
          </CardContent>
        </Card>
        <Card className="border-gray-100">
          <CardContent className="p-4">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs text-gray-500">구독자</span>
            </div>
            <p className="text-xl font-bold">{subscriptions.filter(s => s.subscription_type !== 'free').length}</p>
          </CardContent>
        </Card>
        <Card className="border-gray-100">
          <CardContent className="p-4">
            ...
            <p className="text-xl font-bold">{pendingTransfers}건</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="payments">
        <TabsList className="bg-gray-50 border border-gray-100">
          <TabsTrigger value="payments" className="text-xs">결제 내역 ({payments.length})</TabsTrigger>
        ...
        <TabsContent value="payments" className="space-y-4">
          <div className="flex flex-col sm:flex-row gap-2">
            <div className="relative flex-1">
              ...
              <Input placeholder="이름, 이메일, 전화번호, 주문번호 검색..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 h-9 text-sm" />
            </div>
            <div className="flex gap-1">
              {(['all', 'completed', 'pending'] as const).map(f => (
                <Button key={f} variant={paymentFilter === f ? 'default' : 'outline'} size="sm" onClick={() => setPaymentFilter(f)} className="text-xs h-9">
                  {f === 'all' ? '전체' : f === 'completed' ? '완료만' : '대기만'}
                </Button>
              ))}
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gray-50/50 border-gray-100">
                    ...
                    <TableHead className="text-xs font-medium text-gray-500">상품</TableHead>
                    <TableHead className="text-xs font-medium text-gray-500">금액</TableHead>
                    <TableHead className="text-xs font-medium text-gray-500">결제수단</TableHead>
                    <TableHead className="text-xs font-medium text-gray-500">상태</TableHead>
                    <TableHead className="text-xs font-medium text-gray-500">주문번호</TableHead>
                    <TableHead className="text-xs font-medium text-gray-500">결제일</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {loading ? <SkeletonRows cols={9} /> : filteredPayments.length === 0 ? (
                    <TableRow><TableCell colSpan={9} className="text-center py-8 text-gray-400 text-sm">결제 내역이 없습니다</TableCell></TableRow>
                  ) : (
                      ...
                      <TableRow key={p.id} className="border-gray-50 hover:bg-gray-50/50">
                        <TableCell>
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <div className="text-sm font-medium text-gray-900">{p.display_name || '미설정'}</div>
                              <div className="text-[10px] text-gray-400 font-mono">{p.user_id.slice(0, 12)}...</div>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell className="text-xs text-gray-600">{p.email || '-'}</TableCell>
                        <TableCell className="text-xs text-gray-600">{p.phone || '-'}</TableCell>
                        <TableCell><Badge variant="outline" className="text-[11px]">{getPaymentTypeName(p.subscription_type)}</Badge></TableCell>
                        <TableCell className="text-sm font-semibold">₩{p.amount.toLocaleString()}</TableCell>
                        <TableCell className="text-xs text-gray-600">{p.payment_method || '-'}</TableCell>
                        <TableCell>{getStatusBadge(p.status)}</TableCell>
                        <TableCell className="text-[10px] text-gray-400 font-mono max-w-[120px] truncate">{p.toss_order_id || '-'}</TableCell>
                        <TableCell className="text-[11px] text-gray-500">{format(new Date(p.created_at), 'yy.MM.dd HH:mm')}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
          ...
          <div className="border border-gray-100 rounded-xl overflow-hidden">
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {loading ? <SkeletonRows cols={8} /> : subscriptions.length === 0 ? (
                    <TableRow><TableCell colSpan={8} className="text-center py-8 text-gray-400 text-sm">구독 데이터가 없습니다</TableCell></TableRow>
                  ) : (
                      ...
                      <TableRow key={`${sub.user_id}-${sub.created_at}`} className="border-gray-50 hover:bg-gray-50/50">
                        ...
                        <TableCell>
                          <span className="text-xs text-gray-600">
                            {sub.payment_method === 'toss' ? '토스' : sub.payment_method === 'free_trial' ? '무료체험' : sub.payment_method || '-'}
                          </span>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-[11px] text-gray-500">{format(new Date(sub.created_at), 'yy.MM.dd HH:mm')}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
              ...
              <Table>
                ...
                <TableBody>
                  {bankTransfers.length === 0 ? (
                    <TableRow><TableCell colSpan={8} className="text-center py-8 text-gray-400 text-sm">무통장입금 요청이 없습니다</TableCell></TableRow>
                  ) : (
                    bankTransfers.map((bt) => (
                      <TableRow key={bt.id} className="border-gray-50 hover:bg-gray-50/50">
                        ...
                        <TableCell className="text-[11px] text-gray-500">{format(new Date(bt.created_at), 'yy.MM.dd HH:mm')}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
