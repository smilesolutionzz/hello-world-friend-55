...
const ParentingStyleForm = ({ onComplete, onBack }: ParentingStyleFormProps) => {
  ...
  if (!hasStarted) {
    return (
      <TokenGate
        tokensRequired={10}
        featureName="프리미엄 부모양육태도 검사"
        onProceed={handleStartTest}
        children={
          <div className="space-y-4 mt-6">
            <div>
              <Label htmlFor="childAge" className="text-sm font-medium">자녀 연령 (선택사항)</Label>
              <input
                id="childAge"
                type="number"
                min="1"
            ...
            <div>
              <Label htmlFor="childGender" className="text-sm font-medium">자녀 성별 (선택사항)</Label>
              <select
                id="childGender"
                value={childGender}
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-purple-50/20 to-blue-50/20 relative overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-6 pt-8 pb-16">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground">
              {currentQuestion + 1} / {allQuestions.length} 문항
            </p>
          </div>
            ...
            <CardHeader className="bg-gradient-to-r from-purple-50 to-blue-50">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-sm font-medium text-purple-800">
                  {currentQuestionData.category === 'warmth_acceptance' ? '온정수용' :
                   currentQuestionData.category === 'behavioral_control' ? '행동통제' :
                   currentQuestionData.category === 'psychological_control' ? '심리통제' :
                   currentQuestionData.category === 'autonomy_support' ? '자율성지지' :
                   currentQuestionData.category === 'communication_support' ? '의사소통지지' : 
                   '양육태도'} 영역
                </span>
              </div>
        ...
        <div className="max-w-4xl mx-auto mt-8 flex justify-between items-center">
          ...
          >
            {currentQuestion === allQuestions.length - 1 ? '완료' : '다음'}
