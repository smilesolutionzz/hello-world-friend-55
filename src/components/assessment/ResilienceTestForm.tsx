...
export default function ResilienceTestForm({ onComplete, onBack }: ResilienceTestFormProps) {
  ...
  const analyzeResults = (finalAnswers: Record<string, string>) => {
    ...
    setTimeout(() => {
      ...
      if (totalScore >= 80) {
        resilienceType = "탁월한 회복력 리더";
      } else if (totalScore >= 65) {
        resilienceType = "안정적인 적응자";
      } else if (totalScore >= 50) {
        resilienceType = "성장 중인 회복자";
      } else {
        resilienceType = "회복력 개발 필요";
      }

      // 회복탄력성 수준
      let resilienceLevel: string;
      if (totalScore >= 80) {
        resilienceLevel = "매우 높음";
      } else if (totalScore >= 65) {
        resilienceLevel = "높음";
      } else if (totalScore >= 50) {
        resilienceLevel = "보통";
        ...
        resilienceLevel = "관심 필요";
      }

      // 맞춤 추천
      const recommendations: string[] = [];
      if (categoryScores.stress_recovery < 60) {
        recommendations.push("스트레스 해소를 위한 규칙적인 운동이나 명상을 시작해보세요");
      }
      if (categoryScores.adaptability < 60) {
        recommendations.push("작은 변화부터 시작해 점진적으로 유연성을 높여보세요");
      }
      if (categoryScores.emotional_stability < 60) {
        recommendations.push("감정일기를 써보거나 마음챙김 호흡을 연습해보세요");
      }
      if (categoryScores.social_support < 60) {
        recommendations.push("신뢰할 수 있는 사람과 정기적으로 대화하는 시간을 가져보세요");
      }
      if (categoryScores.purpose_growth < 60) {
        recommendations.push("나의 가치관과 장기 목표를 글로 정리해보세요");
      }
      if (recommendations.length === 0) {
        recommendations.push("현재 회복탄력성이 좋습니다. 지속적인 자기관리로 유지해주세요");
      }
          ...
          <CardContent className="pt-6">
            <div className="flex flex-col items-center space-y-4">
              ...
              <div className="text-center">
                <h3 className="font-semibold text-lg text-emerald-800">회복탄력성 심층 분석 중...</h3>
                <p className="text-emerald-600">5가지 영역을 종합적으로 분석하고 있어요</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <CardHeader className="bg-gradient-to-r from-emerald-500 via-teal-500 to-cyan-500 text-white rounded-t-lg">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <CardTitle className="text-xl">회복탄력성 검사</CardTitle>
              ...
              <div>
                ...
                >
                  {options.map((option, index) => (
                    <div 
                      key={index} 
                      className="flex items-center space-x-2 p-3 rounded-lg hover:bg-emerald-50 transition-colors cursor-pointer border border-transparent hover:border-emerald-200"
                    >
                      <RadioGroupItem value={option.value} id={`resilience-q${currentQuestion}-opt${index}`} />
                      <Label htmlFor={`resilience-q${currentQuestion}-opt${index}`} className="cursor-pointer flex-1">
                        {option.label}
                      </Label>
                    </div>
                  ))}
