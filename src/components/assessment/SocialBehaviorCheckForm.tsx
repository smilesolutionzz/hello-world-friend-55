...
const SocialBehaviorCheckForm: React.FC<SocialBehaviorCheckFormProps> = ({ onComplete, onBack }) => {
  ...
  const categoryColors = {
    social_interaction: "bg-blue-100 text-blue-600",
    play_peer: "bg-green-100 text-green-600",
    communication: "bg-purple-100 text-purple-600",
    behavioral_patterns: "bg-orange-100 text-orange-600",
  ...
  if (!birthDate) {
    return (
      <BirthDateSelector
        testTitle="AIH 사회적 행동 발달 자가체크"
        testSubtitle="SBDS-AIH (Social Behavior Development Self-check)"
        testDescription="일상에서 관찰되는 아이의 사회적 행동 패턴을 체크하는 부모용 자가점검 도구입니다"
    ...
    try {
      ...
      Object.entries(categoryScores).forEach(([category, scores]) => {
        ...
        const avg = validScores.length > 0 
          ...
          : 0;
        // Convert to 0-100 scale (max score per question is 4 * max weight ~1.4 = 5.6)
  ...
  if (isLoading) {
    ...
    return (
      <ModernAnalysisLoading
        title="사회적 행동 발달 분석 중"
        description="AI가 관찰 내용을 종합 분석하고 있습니다..."
        estimatedTime={20}
        icon={Users}
        quotes={quotes}
        insights={insights}
        ...
        <div className="max-w-3xl mx-auto">
          <Card className="border-2 border-green-200 shadow-lg">
            ...
            <CardContent className="space-y-6">
              <div className="text-center">
                <h3 className="text-lg md:text-xl font-medium mb-2 leading-relaxed">
                  {currentQuestion.text}
                </h3>
                <p className="text-sm text-muted-foreground">
                  최근 2-4주간 관찰한 내용을 바탕으로 선택해주세요
                </p>
              </div>
                  ...
                  >
                    <div className="flex items-center w-full">
                      <div className={`w-4 h-4 rounded-full border-2 mr-3 flex-shrink-0 ${
                        ...
                        {answers[currentQuestion.id] === option.value.toString() && (
                          <div className="w-2 h-2 bg-green-600 rounded-full m-auto mt-0.5" />
                        )}
                      </div>
          ...
          <Card className="bg-green-50 border-green-200">
            <CardContent className="pt-6">
              <div className="text-center space-y-2">
                <h4 className="font-semibold text-green-900">📋 자가체크 안내</h4>
                <p className="text-sm text-green-800 leading-relaxed">
                  본 자가체크는 <strong>진단 도구가 아닙니다</strong>. 부모님의 일상 관찰을 바탕으로 
                  아이의 발달 특성을 이해하는 참고 자료입니다. 걱정되는 부분이 있다면 전문가와 상담하세요.
                </p>
              </div>
            </CardContent>
          </Card>
