...
export const WomensHealthTest: React.FC<WomensHealthTestProps> = ({ onComplete, onBack }) => {
  ...
  const calculateResult = () => {
    ...
    const dominantType = Object.entries(constitutionScore)
      .sort(([,a], [,b]) => b - a)[0][0];

    const result = {
      type: 'women_health',
      constitution: dominantType,
      scores,
      constitutionScore,
      answers,
      ...
      <div className="max-w-2xl mx-auto">
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <Button 
              variant="ghost" 
              onClick={onBack || (() => setCurrentQuestion(Math.max(0, currentQuestion - 1)))}
            ...
            <div className="flex items-center space-x-2">
              ...
              <span className="font-medium text-pink-600">여성 건강 체질 분석</span>
            </div>
              ...
              {currentQ.options.map((option) => (
                <div key={option.value} className="flex items-center space-x-2">
                  ...
                  <Label 
                    ...
                    className="flex-1 cursor-pointer p-3 rounded-lg border hover:bg-pink-50 transition-colors"
                  >
                    {option.label}
                  </Label>
                </div>
