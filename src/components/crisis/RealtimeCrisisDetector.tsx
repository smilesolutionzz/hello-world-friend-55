...
export const RealtimeCrisisDetector = () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        if (data.analysis?.immediateActionNeeded) {
          toast({
            title: isEnglish ? '⚠️ Crisis detected' : '⚠️ 위기 상황 감지',
            description: isEnglish ? 'Professional help may be needed.' : '전문가 도움이 필요할 수 있습니다.',
            variant: 'destructive',
          });
        }
      }
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => { analyzeText(text); }, 500);
  ...
  const getLevelText = (level: string) => {
    if (isEnglish) {
      const texts: Record<string, string> = { critical: '🚨 Critical', high: '⚠️ High Risk', medium: 'Caution', low: 'Safe' };
      return texts[level] || level;
    }
    const texts: Record<string, string> = { critical: '🚨 긴급', high: '⚠️ 위험', medium: '주의', low: '안전' };
    return texts[level] || level;
  };
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h3 className="text-lg font-semibold">{isEnglish ? 'Real-time Crisis Detection AI' : '실시간 위기 감지 AI'}</h3>
        ...
        <Textarea
          ...
          placeholder={isEnglish ? 'Type a message to analyze crisis level in real-time...' : '메시지를 입력하면 실시간으로 위기 수준을 분석합니다...'}
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
              <div className="flex items-center gap-3">
                <Badge className={getLevelColor(analysis.crisisLevel)}>{getLevelText(analysis.crisisLevel)}</Badge>
                <span className="text-sm text-muted-foreground">
                  {isEnglish ? 'Risk Score' : '위험 점수'}: {analysis.riskScore}%
                </span>
              </div>
              <div className="flex items-center gap-2">
                <Activity className="w-4 h-4" />
                <span className="text-sm">
                  {isEnglish ? 'Confidence' : '신뢰도'}: {Math.round(analysis.confidence * 100)}%
                </span>
              </div>
            </div>
            ...
            {analysis.matchedKeywords.length > 0 && (
              <div className="p-3 bg-orange-50 dark:bg-orange-950/30 rounded-lg">
                <p className="text-sm font-medium mb-2 text-orange-700 dark:text-orange-300">
                  {isEnglish ? 'Detected risk signals:' : '감지된 위험 신호:'}
                </p>
                <div className="flex flex-wrap gap-2">
                  {analysis.matchedKeywords.map((keyword, idx) => (
                    <Badge key={idx} variant="outline" className="text-orange-600">{keyword}</Badge>
                  ))}
                </div>
              </div>
            )}
            ...
            {aiInsights && (
              <div className="p-4 bg-blue-50 dark:bg-blue-950/30 rounded-lg space-y-3">
                <h4 className="font-medium text-blue-700 dark:text-blue-300 flex items-center gap-2">
                  ...
                  {isEnglish ? 'AI Analysis Results' : 'AI 분석 결과'}
                </h4>
                
                {aiInsights.primaryConcerns?.length > 0 && (
                  <div>
                    <p className="text-sm text-muted-foreground mb-1">{isEnglish ? 'Primary concerns:' : '주요 우려사항:'}</p>
                ...
                {aiInsights.recommendedResponse && (
                  <div className="text-sm">
                    <p className="text-muted-foreground mb-1">{isEnglish ? 'Recommended response:' : '권장 대응:'}</p>
                ...
                {aiInsights.emotionState && (
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {isEnglish ? 'Intensity' : '강도'}: {aiInsights.emotionState.intensity}/10
                    </span>
                  </div>
                )}
              </div>
          ...
          <AlertDescription>
            <div className="space-y-4">
              <p className="font-semibold text-red-700 dark:text-red-300">
                {isEnglish ? 'Immediate help is needed' : '즉각적인 도움이 필요합니다'}
              </p>
              
              <div className="grid gap-3">
                {emergencyContacts.map((contact, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-white dark:bg-background rounded-lg border">
                    <div>
