...
export const SimplifiedFlow = ({ onStepComplete }: SimplifiedFlowProps) => {
  ...
  const handleStartFeature = (feature: typeof coreFeatures[0]) => {
    ...
    if (feature.id === 1) {
      toast({
        title: "🎯 첫 번째 단계 시작!",
        description: "간단한 관찰부터 시작해서 AI 분석의 놀라운 결과를 경험해보세요",
      });
    } else if (feature.id === 2) {
      toast({
        title: "🧠 AI 분석으로 업그레이드",
        description: "관찰 데이터를 기반으로 전문가 수준의 분석을 받아보세요",
      });
    } else if (feature.id === 3) {
      toast({
        title: "👨‍⚕️ 전문가와 직접 상담",
        description: "AI 분석 결과를 바탕으로 전문가의 개인 맞춤 상담을 받아보세요",
      });
    }
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                <Badge variant="outline" className="bg-white">1단계: 무료 관찰 체험</Badge>
                <ArrowRight className="w-4 h-4 text-muted-foreground mt-1" />
                <Badge variant="outline" className="bg-white">2단계: AI 분석 확인</Badge>
                <ArrowRight className="w-4 h-4 text-muted-foreground mt-1" />
                <Badge variant="outline" className="bg-white">3단계: 전문가 상담</Badge>
              </div>
            </div>
          </div>
        </CardContent>
