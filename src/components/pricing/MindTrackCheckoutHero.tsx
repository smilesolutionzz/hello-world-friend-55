      ...
      <Card className="relative overflow-hidden border-2 border-primary/40 shadow-xl bg-gradient-to-br from-white via-blue-50/40 to-purple-50/40">
        ...
        <div className="p-5 sm:p-6 md:p-8">
          ...
          <h2 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 mb-2 break-keep text-center sm:text-left leading-snug">
            {goalLabel ? (
              <>
                ...
                <span className="sm:ml-1">7일 트랙 시작 준비 완료</span>
              </>
            ) : (
              <>7일 마음 변화 트랙 결제</>
            )}
          ...
          <p className="text-sm sm:text-base text-slate-600 mb-6 break-keep text-center sm:text-left">
            결제하면 <strong>오늘부터 7일간</strong> 매일 5분 맞춤 코칭 미션과 변화 리포트가 열립니다. 7일 완주 시 30일 연장 옵션을 안내드려요.
          </p>

          {/* What you get */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 mb-5">
            {[
              { icon: Calendar, text: "매일 5분 맞춤 코칭 미션 (7일)" },
              { icon: Heart, text: "주 1회 마음 상태 변화 리포트" },
              { icon: Sparkles, text: "전문가 코칭 가이드 동봉" },
            ...
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 text-center sm:text-left">
              <div>
                ...
                <div className="flex items-baseline justify-center sm:justify-start gap-1.5 flex-wrap">
                  ...
                  <span className="text-xs sm:text-sm text-slate-600">/ 7일 전체</span>
                </div>
                <div className="text-[11px] sm:text-xs text-emerald-700 font-semibold mt-1">하루 약 ₩{Math.round(TRACK_PRICE / 7).toLocaleString()} · 커피 한 잔보다 저렴</div>
              </div>
          ...
          >
            {loading ? (
              ...
              <span className="break-keep leading-tight">
                ₩{TRACK_PRICE.toLocaleString()} 결제하고 7일 시작하기 →
              </span>
            )}
          </Button>
          ...
          <button
            ...
            className="mt-2 w-full text-xs text-slate-400 hover:text-slate-600 flex items-center justify-center gap-1"
          >
            다른 플랜 비교 <ArrowDown className="w-3 h-3" />
          </button>
        </div>
      </Card>
    </motion.div>
