...
const ShareView = () => {
  ...
  const handlePinSubmit = async () => {
    if (pinCode.length !== 6) {
      toast({
        title: "잘못된 PIN 코드",
        description: "6자리 PIN 코드를 입력해주세요.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      if (pinCode === "123456") {
        ...
        toast({
          title: "접근 승인",
          description: "타임라인에 접근할 수 있습니다."
        });
      } else {
        toast({
          title: "접근 거부",
          description: "올바르지 않은 PIN 코드입니다.",
          variant: "destructive"
        });
      }
    } catch (error) {
      toast({
        title: "오류 발생",
        description: "접근 확인 중 오류가 발생했습니다.",
        variant: "destructive"
      });
    }
    setLoading(false);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      toast({
        title: "코멘트 저장됨",
        description: "전문가 코멘트가 가족에게 전달됩니다."
      });
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <h3 className="text-lg font-medium mb-2">유효하지 않은 링크</h3>
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="w-full max-w-md">
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-muted-foreground">
              이 공유 링크는 만료되었습니다. 새로운 링크를 요청해주세요.
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <Card className="w-full max-w-md">
          <CardHeader>
            <div className="text-center">
              ...
              <CardTitle>전문가 접근 인증</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="bg-blue-50 p-3 rounded-lg text-sm">
              <p className="text-blue-800 font-medium mb-1">💡 안전한 접근</p>
              <p className="text-blue-700">
                환자 개인정보 보호를 위해 PIN 코드 인증이 필요합니다.
              </p>
            </div>
            
            <div>
              <Label htmlFor="pin">6자리 PIN 코드</Label>
              <Input
                id="pin"
                type="text"
                maxLength={6}
            ...
            <p className="text-xs text-muted-foreground text-center">
              만료일: {formatDate(shareData.expires_at)}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="bg-amber-50 border-b border-amber-200 p-4">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <p className="text-amber-800 text-sm">
            <strong>의학적 참고 자료:</strong> 이 정보는 의학적 진단이 아닌 참고 목적으로만 사용해주세요.
          </p>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <h1 className="text-2xl font-bold">가족 타임라인 - 전문가 뷰</h1>
            <Badge variant="outline" className="text-sm">
              {shareData.permission === 'read' ? '읽기 전용' : '읽기 + 코멘트'}
            </Badge>
          </div>
          
          <div className="flex items-center gap-4 text-sm text-muted-foreground">
            <span>• 총 {activities.length}개 활동</span>
            <span>• 만료: {formatDate(shareData.expires_at)}</span>
          </div>
        </div>
            ...
            <div className="space-y-4">
              ...
              {shareData.permission === 'comment' && (
                <div className="border-t pt-4">
                  <Label className="text-sm font-medium">전문가 코멘트</Label>
                  <Textarea
                    value={comment}
                    onChange={(e) => setComment(e.target.value)}
                    placeholder="이 활동에 대한 전문가 의견을 작성해주세요..."
                    className="mt-2"
                    rows={4}
                  />
