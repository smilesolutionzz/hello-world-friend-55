...
const TokenHistory = () => {
  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      console.error('토큰 이력 조회 오류:', error);
      toast({
        title: '이력 조회 실패',
        description: '토큰 이력을 불러오는데 실패했습니다.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const getFeatureTypeLabel = (type: string) => {
    const labels: Record<string, string> = {
      token_consumption: '토큰 소비',
      assessment_tokens: '발달 평가',
      observation_analysis: '관찰일지 분석',
      ai_analysis: 'AI 분석',
      dream_interpretation: '꿈 해석',
      premium_assessment: '프리미엄 평가',
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background py-8 px-4">
      <div className="container mx-auto max-w-4xl">
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold mb-2">토큰 이력</h1>
          <p className="text-muted-foreground">
            토큰 구매 및 사용 내역을 확인할 수 있습니다
          </p>
        </div>
                ...
                <Card key={purchase.id} className="hover:shadow-lg transition-shadow">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-lg">
                            {getPurchaseTypeLabel(purchase.subscription_type)}
                          </CardTitle>
                          <p className="text-sm text-muted-foreground mt-1">
                            {format(new Date(purchase.created_at), 'PPP', { locale: ko })}
                          </p>
                        </div>
                      </div>
                  ...
                  <CardContent>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <span>
                        {format(new Date(purchase.created_at), 'PPP p', { locale: ko })}
                      </span>
                    </div>
                  </CardContent>
                </Card>
            ...
            {usages.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  ...
                  <p className="text-muted-foreground">사용 이력이 없습니다</p>
                </CardContent>
              </Card>
            ) : (
              usages.map((usage) => (
                <Card key={usage.id} className="hover:shadow-lg transition-shadow">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-red-100 rounded-lg">
                          {getFeatureTypeIcon(usage.feature_type)}
                        </div>
                        <div>
                          <CardTitle className="text-lg">
                            {getFeatureTypeLabel(usage.feature_type)}
                          </CardTitle>
                          <p className="text-sm text-muted-foreground mt-1">
                            {format(new Date(usage.usage_date), 'PPP', { locale: ko })}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                  ...
                  <CardContent>
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      <Calendar className="w-4 h-4" />
                      <span>
                        {format(new Date(usage.created_at), 'PPP p', { locale: ko })}
                      </span>
                    </div>
                  </CardContent>
                </Card>
          ...
          <CardContent className="py-4">
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                <p className="font-semibold text-foreground mb-1">투명한 토큰 관리</p>
                <p className="text-muted-foreground">
                  모든 토큰 구매 및 사용 내역이 투명하게 기록됩니다. 
                  언제든지 이 페이지에서 확인할 수 있습니다.
                </p>
              </div>
            </div>
          </CardContent>
