...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `당신은 20년 경력의 성격심리학 전문가이자 HEXACO 모델의 권위자입니다.

HEXACO 모델은 Lee와 Ashton이 개발한 6차원 성격 모델로, 기존 Big5 모델에 정직-겸손성 차원을 추가하여 성격을 더 포괄적으로 설명합니다.
...
- 내담자의 강점을 발견하고 격려`;
    ...
    const userPrompt = `다음은 HEXACO 성격 검사 결과입니다:
