...
당신은 임상심리학 전문가입니다. Beck 우울척도(BDI-II) 기반 체크 결과를 참고 분석해주세요.
...
다음 영역을 포함하여 참고용 분석을 제공해주세요 (의학적 진단 아님):

1. **점수 해석**: 현재 점수가 의미하는 바와 우울감 수준

2. **현재상태 패턴 분석**: 
   - 주요 우울감 영역별 참고 분석 (인지적, 정서적, 신체적, 행동적 어려움)
   - 특히 높은 점수를 보인 문항들의 의미

3. **주의사항**: 
   - 자해나 자살 생각 등 위험 신호 확인
   - 일상생활 어려움 정도
...
분석은 따뜻하고 공감적인 톤으로 작성하되, 참고용임을 명시하고 전문적이어야 합니다. 
약 800-1000자 분량으로 상세히 작성해주세요.
모든 분석 내용 앞에 "참고용 의견으로" 등의 문구를 포함해주세요.
`;
...
현재 체크 결과에 따르면 ${results?.severity || '분석 중'} 수준으로 확인되었습니다.
