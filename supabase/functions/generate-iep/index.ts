...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (observationLogs.length > 0) {
      observationAnalysis = `
## 관찰일지 기반 종합 분석 (${observationLogs.length}개 기록)
${observationLogs.map((log, index) => `
### ${index + 1}번째 관찰기록 (${log.session_name})
- **관찰 대상**: ${log.target_name}
- **관찰일**: ${new Date(log.created_at).toLocaleDateString('ko-KR')}
- **AI 분석 결과**: 
  ${log.ai_analysis ? `
  - 상황: ${log.ai_analysis.situation || ''}
  - 주요 포인트: ${log.ai_analysis.points ? log.ai_analysis.points.join(', ') : ''}
  - 개선사항: ${log.ai_analysis.tips ? log.ai_analysis.tips.join(', ') : ''}
  - 주의사항: ${log.ai_analysis.alerts ? log.ai_analysis.alerts.join(', ') : ''}
  ` : '분석 대기중'}
- **원본 관찰 내용**: ${log.raw_data || ''}
`).join('\n')}

이 관찰일지들을 종합하여 학생의 현재 상태, 강점, 필요한 지원을 구체적으로 파악하고 IEP에 반영해주세요.
`
    }
