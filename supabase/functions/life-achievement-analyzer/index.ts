...
serve(async (req) => {
  ...
  try {
    ...
    // 분석 생성
    const analysisPrompt = `사용자의 인생 업적 달성률을 분석해주세요.
...
다음 형식으로 분석해주세요 (각 섹션을 명확히 구분):

1. 종합평가 (100자): 현재 인생 단계에 대한 따뜻하고 격려하는 평가

2. 최고 강점 (80자): 가장 잘하고 있는 영역과 진심 어린 칭찬

3. 개선 영역 (80자): 부드럽게 개선이 필요한 영역 언급

4. 다음 목표 (각 목표당 50자):
   - 첫 번째 목표: [구체적 실행 가능한 목표]
   - 두 번째 목표: [구체적 실행 가능한 목표]  
   - 세 번째 목표: [구체적 실행 가능한 목표]

5. 응원 메시지 (60자): 힘이 되는 마무리 메시지

친근하고 따뜻한 톤으로 작성해주세요. 숫자나 특수문자로 시작하지 말고 자연스럽게 작성해주세요.`;
