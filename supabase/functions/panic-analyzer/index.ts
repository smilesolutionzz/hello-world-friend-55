...
serve(async (req) => {
  ...
  try {
    ...
    const targetLabel = isChild ? '아동 (7-12세)' : '성인';
    ...
    const response = await fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `당신은 20년 경력의 임상심리학 박사이자 불안장애/공황장애 전문가입니다. ${isChild ? '아동 불안' : '성인 불안/공황장애'} 자가진단 결과를 바탕으로 매우 전문적이고 상세한 분석을 제공합니다. 학술적 근거를 바탕으로 정확하고 실용적이며 희망적인 관점을 유지하세요. 각 섹션을 최소 200자 이상으로 풍부하게 작성하세요.`
          },
