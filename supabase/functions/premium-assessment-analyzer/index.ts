...
**7차원 점수 결과:**
${Object.entries(actualResults).map(([dimension, score]) => `- ${dimension}: ${score}점/7점`).join('\n')}
...
**5요인 점수 결과:**
${Object.entries(actualResults).map(([factor, score]) => `- ${factor}: ${score}점/7점`).join('\n')}
...
**번아웃 영역별 점수:**
${Object.entries(actualResults).map(([domain, score]) => `- ${domain}: ${score}점/7점`).join('\n')}
