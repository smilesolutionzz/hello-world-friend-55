// Test endpoint: sends a sample daily coaching email with quality-filtered YouTube recommendations
// + preview validation (link/thumbnail health check)
...
const SEARCH_TERMS_BY_CATEGORY: Record<string, string[]> = {
  stress: ["마음챙김 명상 5분", "스트레스 해소 호흡법", "이완 명상 가이드"],
...
async function searchYouTube(q: string, max = 5): Promise<string[]> {
  const url = `https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=${max}&relevanceLanguage=ko&regionCode=KR&order=relevance&safeSearch=strict&videoEmbeddable=true&q=${encodeURIComponent(q)}&key=${YOUTUBE_API_KEY}`;
...
async function fetchDetails(ids: string[]): Promise<any[]> {
...
async function fetchYouTubeVideos(category: string, mission: string, exclude: Set<string>): Promise<YTVideo[]> {
  ...
  for (const term of terms.slice(0, 3)) {
    try {
      const ids = await searchYouTube(term, 5);
      for (const id of ids) {
        if (exclude.has(id)) continue;
        if (!idMap.has(id)) idMap.set(id, term);
      }
    } catch (e) { console.error("yt search err", term, e); }
    if (idMap.size >= 12) break;
  }

  const details = await fetchDetails([...idMap.keys()].slice(0, 20));
  const scored: Array<YTVideo & { score: number }> = [];
  for (const it of details) {
    const id = it.id;
    const views = parseInt(it.statistics?.viewCount || "0");
    const dur = parseISO8601Duration(it.contentDetails?.duration || "PT0S");
    if (views < MIN_VIEW_COUNT) continue;
    if (dur < MIN_DURATION_SECONDS || dur > MAX_DURATION_SECONDS) continue;
    const term = idMap.get(id) || terms[0];
    scored.push({
      videoId: id,
      title: it.snippet.title,
      channelTitle: it.snippet.channelTitle,
      thumbnail: it.snippet.thumbnails?.medium?.url || it.snippet.thumbnails?.default?.url,
      reason: `오늘 미션 "${mission.slice(0, 28)}…"을 ${Math.round(dur / 60)}분 안에 익히도록 도와주는 ${term} 가이드`,
      viewCount: views,
      durationSeconds: dur,
      score: Math.log10(views + 1) * 10 + (dur <= 600 ? 5 : 0),
    });
  }
...
function buildHtml(nickname: string, videos: YTVideo[]) {
  ...
  const missionSummary = "5분 호흡으로 오늘의 긴장을 풀어내기";
  ...
  const actionsHtml = keyActions.map((a, i) => `
    <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:8px;">
      <div style="flex:0 0 22px;height:22px;border-radius:99px;background:#0f172a;color:#fff;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center;">${i + 1}</div>
      <div style="font-size:14px;color:#0f172a;line-height:1.5;">${a}</div>
    </div>`).join('');

  const videosHtml = videos.length === 0 ? '' : `
    <div style="margin:0 0 28px;">
      <div style="font-size:11px;letter-spacing:0.16em;color:#64748b;text-transform:uppercase;margin-bottom:12px;">04 · 오늘의 추천 영상</div>
      ${videos.map(v => `
          ...
          <table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;">
            <tr>
              ...
              <td style="vertical-align:top;">
                ...
                <div style="font-size:11px;color:#64748b;margin-bottom:6px;">${v.channelTitle}${v.durationSeconds ? ` · ${Math.round(v.durationSeconds/60)}분` : ''}${v.viewCount ? ` · 조회 ${Math.round(v.viewCount/1000)}K` : ''}</div>
...
<body style="margin:0;padding:0;background:#ffffff;font-family:-apple-system,'Pretendard Variable',Inter,sans-serif;color:#0f172a;">
  <div style="max-width:560px;margin:0 auto;padding:32px 24px;">
    ...
    <div style="height:4px;background:#f1f5f9;border-radius:99px;margin:18px 0 28px;overflow:hidden;">
      <div style="width:${progressPct}%;height:100%;background:linear-gradient(90deg,#0f172a,#3b82f6);"></div>
    </div>

    <div style="background:#0f172a;color:#fff;border-radius:12px;padding:18px 20px;margin-bottom:24px;">
      <div style="font-size:11px;letter-spacing:0.16em;color:#94a3b8;text-transform:uppercase;margin-bottom:6px;">오늘의 한 줄</div>
      <div style="font-size:17px;font-weight:600;line-height:1.5;">${missionSummary}</div>
    </div>

    <div style="border-left:3px solid #0f172a;padding:4px 0 4px 16px;margin-bottom:24px;">
      <div style="font-size:11px;letter-spacing:0.16em;color:#64748b;text-transform:uppercase;margin-bottom:8px;">01 · 오늘의 미션</div>
      <div style="font-size:15px;line-height:1.65;color:#0f172a;font-weight:500;">${mission}</div>
    </div>

    <div style="margin-bottom:28px;">
      <div style="font-size:11px;letter-spacing:0.16em;color:#64748b;text-transform:uppercase;margin-bottom:12px;">02 · 핵심 행동 3</div>
      ${actionsHtml}
    </div>

    <div style="background:#f8fafc;border-radius:12px;padding:20px 22px;margin-bottom:28px;">
      <div style="font-size:11px;letter-spacing:0.16em;color:#64748b;text-transform:uppercase;margin-bottom:10px;">03 · 임상적 근거</div>
...
serve(async (req) => {
  ...
  try {
    const { to, nickname, user_id, dry_run } = await req.json();
    if (!to && !dry_run) throw new Error("to required");
    ...
    // Load exclude set + preferences if user_id provided
    ...
    if (user_id) {
      ...
      const { data } = await supa.from("daily_coaching_video_history")
        .select("video_id").eq("user_id", user_id).gte("sent_date", cutoff.toISOString().slice(0, 10));
      exclude = new Set((data || []).map((r: any) => r.video_id));
    }

    const mission = "오늘은 마음챙김 호흡을 5분간 시도하고, 호흡 전후의 긴장감을 1~10점으로 기록해보세요.";
    const rawVideos = await fetchYouTubeVideos("stress", mission, exclude);
    const videos = await validateVideoLinks(rawVideos);

    const validation = videos.map((v) => ({
      videoId: v.videoId,
      title: v.title,
      watchUrl: v.watchUrl,
      thumbnail: v.thumbnail,
      linkOk: v.linkOk,
      thumbnailOk: v.thumbnailOk,
      viewCount: v.viewCount,
      durationSeconds: v.durationSeconds,
      excluded: exclude.has(v.videoId),
    }));

    if (dry_run) {
      return new Response(JSON.stringify({ success: true, dry_run: true, videoCount: videos.length, validation, excludedCount: exclude.size }),
    ...
    return new Response(JSON.stringify({ success: true, videoCount: videos.length, validation, excludedCount: exclude.size, result }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (e) {
    console.error(e);
