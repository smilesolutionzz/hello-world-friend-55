-- Create custom types
CREATE TYPE app_role AS ENUM ('admin', 'expert', 'viewer');
...
-- Create user_referrals policies
CREATE POLICY "Users can view their own referrals" ON public.user_referrals
FOR SELECT USING (referrer_profile_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()));
...
-- Create families policies
CREATE POLICY "Users can create families" ON public.families
FOR INSERT WITH CHECK (created_by IN (SELECT id FROM profiles WHERE user_id = auth.uid()));

CREATE POLICY "Family creators can update their families" ON public.families
FOR UPDATE USING (created_by IN (SELECT id FROM profiles WHERE user_id = auth.uid()));

CREATE POLICY "Family members can view their families" ON public.families
FOR SELECT USING (id IN (SELECT fm.family_id FROM family_members fm JOIN profiles p ON fm.profile_id = p.id WHERE p.user_id = auth.uid()));
...
-- Create grant_welcome_tokens function
