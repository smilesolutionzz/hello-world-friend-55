-- 매일 토큰 지급을 위한 시스템 설정 (크론 제외)

-- user_tokens 테이블에 마지막 일일 토큰 지급 날짜 추가
...
BEGIN
  ...
  ON CONFLICT (user_id) 
  DO UPDATE SET 
    current_tokens = user_tokens.current_tokens + 10,
    total_purchased = user_tokens.total_purchased + 10,
    last_daily_bonus_date = CURRENT_DATE;
  RETURN NEW;
END;
$$;